...
handler.on('push', function (data) {
  if (data.payload.ref.split('/')[1] === 'heads') {
    createDiscordMessage(`\n **\`${data.payload.repository.full_name}\`**`, 
	`\n \`${data.payload.sender.login}\` pushed **${data.payload.commits.length} commit${data.payload.commits.length === 1 ? '' : 's'}** to ${data.payload.ref.split('/')[2]}${data.payload.commits.map(c => `\n \`\`\`${c.id.substr(0, 7)} - ${c.message.split('\n')[0]}\`\`\` `).join('')}\n<${data.payload.repository.html_url}/${data.payload.head_commit.id.substr(0, 7)}>`);
  }
});
