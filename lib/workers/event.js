...
var githubHandler = function(req, res) {
    handler(req, res, function(err) {
        if (err) {
            logger.debug('Github Handler Error: ' + err);
        }
        res.statusCode = 404;
...
var middleware = route.comp([route.wrap(config.web.path || '/webhook', githubHandler), route.wrap('/ping', pingHandler)]);
...
setInterval(function() {
    http.get(config.web.url || 'http://' + (config.web.cname || 'localhost') + '/ping');
}, 22 * (60 * 1000));
...
bot.on('ready', function() {
    ...
    server.listen(config.web.port || 8080);
    console.log(format('--Webhook server listening on port: [%s]--', [config.web.port || 8080]));
});
...
bot.on('error', function(err) {
    if (err) {
        logger.debug(format('Bot Error: %s\n%s', [err, err.stack]));
    }
});
  ...
  switch (data.payload.action) {
    ...
    case 'reopened': {
      ...
      if (modifiedLabels.issues.labeled.UpdateAt !== data.payload.issue.updated_at && modifiedLabels.issues.labeled.ID !== data.payload.issue.id) {
        setTimeout(function () {
          createDiscordMessage(data.payload.repository.full_name, `\`${data.payload.sender.login}\` **added** labels \`${modifiedLabels.issues.labeled.stored.trim()}\` to issue \`#${data.payload.issue.number}\`: \n\`\`\`${data.payload.issue.title.replace(/```/g, '``')}\n\`\`\``, data.payload.issue.html_url);
    ...
    case 'unlabeled':
      if (modifiedLabels.issues.unlabeled.UpdateAt !== data.payload.issue.updated_at && modifiedLabels.issues.unlabeled.ID !== data.payload.issue.id) {
        setTimeout(function () {
          createDiscordMessage(data.payload.repository.full_name, `\`${data.payload.sender.login}\` **removed** labels \`${modifiedLabels.issues.unlabeled.stored.trim()}\` from issue \`#${data.payload.issue.number}\`: \n\`\`\`${data.payload.issue.title.replace(/```/g, '``')}\n\`\`\``, data.payload.issue.html_url);
  ...
  switch (data.payload.action) {
    ...
    case 'labeled':
      if (modifiedLabels.pullRequest.labeled.UpdateAt !== data.payload.pull_request.updated_at && modifiedLabels.pullRequest.labeled.ID !== data.payload.pull_request.id) {
        setTimeout(function () {
          createDiscordMessage(data.payload.repository.full_name, `\`${data.payload.sender.login}\` **added** labels \`${modifiedLabels.pullRequest.labeled.stored.trim()}\` to pull request \`#${data.payload.pull_request.number}\`: \n\`\`\`${data.payload.pull_request.title.replace(/```/g, '``')}\n\`\`\``, data.payload.pull_request.html_url);
    ...
    case 'unlabeled':
      if (modifiedLabels.pullRequest.unlabeled.UpdateAt !== data.payload.pull_request.updated_at && modifiedLabels.pullRequest.unlabeled.ID !== data.payload.pull_request.id) {
        setTimeout(function () {
          createDiscordMessage(data.payload.repository.full_name, `\`${data.payload.sender.login}\` **removed** labels \`${modifiedLabels.pullRequest.unlabeled.stored.trim()}\` from pull request \`#${data.payload.pull_request.number}\`: \n\`\`\`${data.payload.pull_request.title.replace(/```/g, '``')}\n\`\`\``, data.payload.pull_request.html_url);
