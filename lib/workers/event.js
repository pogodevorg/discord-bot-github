...
var modifiedLabels = {issues: {labeled: {stored: '', UpdateAt: '', ID: ''}, unlabeled: {stored: '', UpdateAt: '', ID: ''}}, pullRequest: {labeled: {stored: '', UpdateAt: '', ID: ''}, unlabeled: {stored: '', UpdateAt: '', ID: ''}}};
...
handler.on('pull_request', function (data) {
  switch (data.payload.action) {
    ...
    case 'labeled':
      ...
      break;
    case 'unlabeled':
      if (modifiedLabels.pullRequest.unlabeled.UpdateAt !== data.payload.pull_request.updated_at && modifiedLabels.pullRequest.unlabeledID !== data.payload.pull_request.id) {
        setTimeout(function () {
          createDiscordMessage(data.payload.repository.full_name, `\`${data.payload.sender.login}\` **added** labels \`${modifiedLabels.pullRequest.unlabeled.stored.trim()}\` to pull request \`#${data.payload.pull_request.number}\`: \`\`\`${data.payload.pull_request.title.replace(/```/g, '``')}\n\`\`\``, data.payload.pull_request.html_url);
