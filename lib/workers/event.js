...
handler.on('push', function (data) {
  if (data.payload.commits.length !== 0) {
    if (data.payload.ref.split('/')[1] === 'heads') {
      ...
      logger.log(format('[%s] %s pushed %s commit(s) to [Commit: %s].', [data.payload.repository.full_name, data.payload.sender.login, data.payload.commits.length, data.payload.head_commit.id.substr(0, 7)]));
	}
  }
});
