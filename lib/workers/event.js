...
function createDiscordMessage(repo, text) {
    bot.createMessage(config.discord.channelID, `**${repo}**\n${text}`.substr(0, 2000));
}
...
handler.on('issue_comment', function (data) {
  ...
  logger.log(format('[%s] %s **%s** a comment on issue [#%s |Title: %s].', [data.payload.repository.full_name, data.payload.sender.login, data.payload.action, data.payload.issue.number, data.payload.pull_request.title]));
});
...
handler.on('issues', function (data) {
  switch (data.payload.action) {
    case 'opened':
      ...
      logger.log(format('[%s] %s **opened** issue [#%s | Title: %s].', [data.payload.repository.full_name, data.payload.issue.user.login, data.payload.issue.number, data.payload.pull_request.title]));
      break;
    case 'reopened': {
      ...
      logger.log(format('[%s] %s **re-opened** issue [#%s | Title: %s].', [data.payload.repository.full_name, data.payload.sender.login, data.payload.issue.number, data.payload.pull_request.title]));
      break;
    } case 'closed': {
      let labels = data.payload.issue.labels.map(i => `\`[${i.name}]\``).join(' ');
      createDiscordMessage(data.payload.repository.full_name, `\`${data.payload.sender.login}\` **closed** issue \`#${data.payload.issue.number}\`: \`\`\`\n${data.payload.issue.title.replace(/```/g, '``')} ${labels}\n\`\`\``);
      logger.log(format('[%s] %s **closed** issue [#%s | Title: %s].', [data.payload.repository.full_name, data.payload.sender.login, data.payload.issue.number, data.payload.pull_request.title]));
      break;
    } case 'labeled':
      ...
      logger.log(format('[%s] %s **added** label [%s] to issue [#%s | Title: %s].', [data.payload.repository.full_name, data.payload.sender.login, data.payload.label.name, data.payload.issue.number, data.payload.pull_request.title]));
      break;
    case 'unlabeled':
      if (modifiedLabels.issues.unlabeled.UpdateAt !== data.payload.issue.updated_at && modifiedLabels.issues.unlabeledID !== data.payload.issue.id) {
        setTimeout(function () {
          createDiscordMessage(data.payload.repository.full_name, `\`${data.payload.sender.login}\` **added** labels \`${modifiedLabels.issues.unlabeled.stored.trim()}\` to issue \`#${data.payload.issue.number}\`: \`\`\`${data.payload.issue.title.replace(/```/g, '``')}\n\`\`\``);
      ...
      logger.log(format('[%s] %s **removed** label [%s] from issue [#%s | Title: %s].', [data.payload.repository.full_name, data.payload.sender.login, data.payload.label.name, data.payload.issue.number, data.payload.pull_request.title]));
      break;
    // no default
  }
});
...
handler.on('pull_request', function (data) {
  switch (data.payload.action) {
    ...
    case 'edited':
      ...
      break;
    case 'labeled':
      if (modifiedLabels.pull_request.labeled.UpdateAt !== data.payload.pull_request.updated_at && modifiedLabels.pull_request.labeled.ID !== data.payload.pull_request.id) {
        setTimeout(function () {
          createDiscordMessage(data.payload.repository.full_name, `\`${data.payload.sender.login}\` **added** labels \`${modifiedLabels.pull_request.labeled.stored.trim()}\` to pull request \`#${data.payload.pull_request.number}\`: \`\`\`${data.payload.pull_request.title.replace(/```/g, '``')}\n\`\`\``);
          modifiedLabels.pull_request.labeled.stored = '';
        }, 500);
      }
      modifiedLabels.pull_request.labeled.stored += `[${data.payload.label.name}] `;
      modifiedLabels.pull_request.labeled.UpdateAt = data.payload.pull_request.updated_at;
      modifiedLabels.pull_request.labeled.ID = data.payload.pull_request.id;
      ...
      break;
    case 'unlabeled':
      if (modifiedLabels.pull_request.unlabeled.UpdateAt !== data.payload.pull_request.updated_at && modifiedLabels.pull_request.unlabeledID !== data.payload.pull_request.id) {
        setTimeout(function () {
          createDiscordMessage(data.payload.repository.full_name, `\`${data.payload.sender.login}\` **added** labels \`${modifiedLabels.pull_request.unlabeled.stored.trim()}\` to pull request \`#${data.payload.pull_request.number}\`: \`\`\`${data.payload.pull_request.title.replace(/```/g, '``')}\n\`\`\``);
          modifiedLabels.pull_request.unlabeled.stored = '';
        }, 500);
      }
      modifiedLabels.pull_request.unlabeled.stored += `[${data.payload.label.name}] `;
      modifiedLabels.pull_request.unlabeled.UpdateAt = data.payload.pull_request.updated_at;
      modifiedLabels.pull_request.unlabeled.ID = data.payload.pull_request.id;
