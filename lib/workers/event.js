...
handler.on('issues', function (data) {
  switch (data.payload.action) {
    ...
    case 'reopened': {
      ...
      if (prevIssueUpdateAt !== data.payload.issue.updated_at && prevIssueID !== data.payload.issue.id) {
        createDiscordMessage(data.payload.repository.full_name, `\`${data.payload.sender.login}\` **added** labels to issue \`#${data.payload.issue.number}\`: \`\`\`${data.payload.issue.title.replace(/```/g, '``')}\n\`\`\``);
      }
      setTimeout(bot.createMessage(config.discord.channelID, `[${data.payload.label.name}]`), 100);
      prevIssueUpdateAt = data.payload.issue.updated_at;
      prevIssueID = data.payload.issue.id;
    ...
    case 'unlabeled':
      if (prevIssueUpdateAt !== data.payload.issue.updated_at && prevIssueID !== data.payload.issue.id) {
        createDiscordMessage(data.payload.repository.full_name, `\`${data.payload.sender.login}\` **removed** labels from issue \`#${data.payload.issue.number}\`: \`\`\`\n${data.payload.issue.title.replace(/```/g, '``')}\n\`\`\``);
      }
      setTimeout(bot.createMessage(config.discord.channelID, `\`[${data.payload.label.name}]\``), 100);
      prevIssueUpdateAt = data.payload.issue.updated_at;
      prevIssueID = data.payload.issue.id;
