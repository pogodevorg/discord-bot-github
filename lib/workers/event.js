...
handler.on('commit_comment', function (data) {
  createDiscordMessage(data.payload.repository.full_name, `${data.payload.comment.user.login} _${data.payload.action}_ a comment on \`${data.payload.comment.commit_id.substr(0, 7)}\`: \`\`\`\n${data.payload.comment.body.length > textLimit ? data.payload.comment.body.replace(/```/g, '``').substr(0, textTruncateTo) + ' [...]' : data.payload.comment.body.replace(/```/g, '``')}\n\`\`\` `);
});
...
handler.on('push', function (data) {
  if (data.payload.commits.length !== 0) {
    if (data.payload.ref.split('/')[1] === 'heads') {
      ...
      logger.log(format('<%s> %s pushed to [%s]', [data.payload.head_commit.id.substr(0, 7), data.payload.sender.login, data.payload.repository.full_name]));
	}
  }
});
