  ...
  switch (data.payload.action) {
    ...
    case 'unlabeled':
      if (modifiedLabels.issues.unlabeled.UpdateAt !== data.payload.issue.updated_at && modifiedLabels.issues.unlabeled.ID !== data.payload.issue.id) {
        setTimeout(function () {
          createDiscordMessage(data.payload.repository.full_name, `\`${data.payload.sender.login}\` **removed** labels \`${modifiedLabels.issues.unlabeled.stored.trim()}\` to issue \`#${data.payload.issue.number}\`: \`\`\`${data.payload.issue.title.replace(/```/g, '``')}\n\`\`\``, data.payload.issue.html_url);
  ...
  switch (data.payload.action) {
    ...
    case 'unlabeled':
      if (modifiedLabels.pullRequest.unlabeled.UpdateAt !== data.payload.pull_request.updated_at && modifiedLabels.pullRequest.unlabeled.ID !== data.payload.pull_request.id) {
        setTimeout(function () {
          createDiscordMessage(data.payload.repository.full_name, `\`${data.payload.sender.login}\` **removed** labels \`${modifiedLabels.pullRequest.unlabeled.stored.trim()}\` to pull request \`#${data.payload.pull_request.number}\`: \`\`\`${data.payload.pull_request.title.replace(/```/g, '``')}\n\`\`\``, data.payload.pull_request.html_url);
